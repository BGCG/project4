...
* [views.py](https://res.cloudinary.com/delase5lw/image/upload/v1685358475/viewsv3-PEP8_xfdfec.jpg)
* [models.py](https://res.cloudinary.com/delase5lw/image/upload/v1685358483/modelsv2-PEP8_zcbjve.jpg)
...
One instance I experienced an issue when creating the ContactRequest model, where I wanted to add a first name and last name field to the contact form but when I added it I kept on getting an error when trying to apply the migrations `You are trying to add a non-nullable field`. I believe what happened was there were already contact requests in the database that did not have first name or last name fields which could not be blank, therefore this would be an integrity error. I had a look online to resolve this as even when I commented out the model or set a default value and tried to update the migrations, I was still getting the same error. I found a [post](https://www.linkedin.com/pulse/how-do-i-reset-django-migration-nitin-raturi/?trk=pulse-article_more-articles_related-content-card) which suggested that you can drop the whole database and start again. This seemed like quite an extreme measure so I felt uncomfortable doing this, therefore I contacted Tutor support to see if they could help me find an alternative solution. Sean from Tutor support dropped the table for the ContactRequest model which resolved the issue. 
...
There were a couple of future improvements that could be made to this site. One being that when the user enters an invalid input into the create a post form and tries to submit it, the data in the form is saved but not the image (if they uploaded one). I believe this would enhance the user experience but I could not implement this due to time constraints. If I was to do this I would set up functionality for the image to be uploaded into a temporary folder on Cloudinary. Secondly, due to the ability of users to create a draft post, the publication date will be set to the time that post was created, not when it is set to published and approved by the admin team. I would have implemented this by saving a new publication date when the post only is approved and set to published but due to time constraints I could not implement this. Lastly, it would have been good for me to specify the maximum preparation time value, however I encountered some issues with handling the errors when the value exceeded the maximum number of minutes. Due to time constraints, I could not implement the necessary code to handle the error handling for values that are too large. If I had time, I would implement this error handling by writing a conditional statement in my create post and edit post view and trigger a user friendly message if the values were out of the specified limits.
